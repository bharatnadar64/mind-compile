  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-black flex flex-col items-center justify-center gap-4">
        ...
        <p className="text-emerald-500 font-mono tracking-widest text-xs animate-pulse uppercase">Syncing_Submission_Nodes...</p>
      </div>
    );
  }
    ...
    <div className="relative min-h-screen bg-black text-slate-300 font-mono p-4 sm:p-10 overflow-hidden">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8 mb-16 border-b border-white/5 pb-10 relative z-10">
        <div className="space-y-2">
          <h1 className="text-4xl md:text-6xl font-black tracking-tighter text-white glitch" data-text="TRANSMISSION_LEDGER">
            TRANSMISSION_LEDGER
          </h1>
          <div className="flex items-center gap-3">
            <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
            <p className="text-emerald-500/60 text-[10px] font-black uppercase tracking-[0.4em]">LIVE_INTELLIGENCE_FEED</p>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
          <input
            ...
            className="bg-black/50 border border-white/10 px-6 py-4 text-xs font-black tracking-[0.2em] text-emerald-400 focus:border-emerald-500/50 focus:outline-none transition-all w-full md:w-80"
          />
          <select
            value={roundFilter}
            onChange={(e) => setRoundFilter(e.target.value)}
            className="bg-black/50 border border-white/10 px-6 py-4 text-xs font-black tracking-[0.2em] text-emerald-400 focus:border-emerald-500/50 focus:outline-none transition-all"
          >
          ...
          filtered.map((s) => (
            ...
            >
              <div className="flex flex-col lg:flex-row justify-between gap-8">
                ...
                <div className="flex-1 space-y-6">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-emerald-500/10 border border-emerald-500/30 flex items-center justify-center text-xl font-black text-emerald-400">
                      {s.name?.charAt(0).toUpperCase()}
                    </div>
                    <div>
                      <h2 className="text-lg font-black text-white tracking-widest uppercase">{s.name || "UNKNOWN_NODE"}</h2>
                      <p className="text-[10px] font-mono text-slate-500 tracking-widest uppercase">{s.email || "NO_ID_ATTACHED"}</p>
                    </div>
                  </div>

                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-6">
                    <div className="space-y-1">
                      <span className="text-[9px] font-mono text-slate-500 tracking-widest uppercase">PHASE</span>
                      <p className="text-sm font-black text-white">ROUND_{s.round || "0"}</p>
                    </div>
                    <div className="space-y-1">
                      <span className="text-[9px] font-mono text-slate-500 tracking-widest uppercase">TARGET_PROBLEM</span>
                      <p className="text-sm font-black text-white truncate max-w-[150px] uppercase">{s.problemTitle || "UNDEFINED"}</p>
                    </div>
                    <div className="space-y-1">
                      <span className="text-[9px] font-mono text-slate-500 tracking-widest uppercase">YIELD_SCORE</span>
                      <p className="text-sm font-black text-emerald-400 tabular-nums">+{s.scoreAwarded ?? 0}_PTS</p>
                    </div>
                    <div className="space-y-1">
                      <span className="text-[9px] font-mono text-slate-500 tracking-widest uppercase">TIMESTAMP</span>
                ...
                <div className="flex flex-col sm:flex-row lg:flex-col justify-end gap-3 min-w-[200px]">
                    ...
                    return (
                      <>
                        <button
                          ...
                          className={`py-3 px-6 text-[10px] font-black tracking-widest transition-all uppercase ${
                  ...
                  <button
                    ...
                    className="py-3 px-6 bg-white/[0.03] border border-white/10 text-slate-400 hover:text-white hover:border-white/30 transition-all font-black text-[10px] tracking-widest uppercase"
                  >
                    EXTRACT_CODE_
                  </button>
                </div>
              ...
              <div className="mt-8 border border-white/5 bg-black/40 p-6 relative group/code">
                ...
                <span className="text-[8px] font-black text-slate-700 tracking-[0.5em] mb-4 block uppercase underline underline-offset-4">PAYLOAD_BUFFER_SOURCE</span>
      ...
      <div className="mt-16 flex flex-col sm:flex-row justify-between items-center gap-4 text-[10px] font-mono text-slate-500 tracking-[0.3em] border-t border-white/5 pt-8 relative z-10">
        ...
        <div className="text-emerald-500/60 animate-pulse">SYSTEM_INTACT // STANDBY_FOR_NEXT_TRANSMISSION</div>
      </div>
    </div>
  );
};
